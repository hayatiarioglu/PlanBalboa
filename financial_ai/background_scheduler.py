...
class BackgroundScheduler:
    ...
    1. 10:15 Saf Matematiksel Fiyat Sentineli (P_10:15 Anlık Fiyat ile Crash Miss Bug'sız).
    2. 18:15 Gün Sonu 19-Modüllü EOD Analizi ve SHAP Sürücü Analizi.
    3. Cold-Start Fallback (P_{t-1} = P_t) ve Hysteresis Buffer.
    4. 9-Durumlu Eksiksiz Sinyal Konsolidasyon Matrisi (Dead Cat Bounce Zırhlı).
    """

    # 9-Durumlu Birleşik Sinyal Konsolidasyon Matrisi (Terminal Safe Text)
    ...
    def initialize_models(self):
        """Modeli başlatır ve ağırlıkları dondurur."""
    ...
    def evaluate_1015_gap_sentinel(self, df_live: pd.DataFrame, ticker: str) -> Optional[Dict[str, Any]]:
        """
        10:15 Gap Sentinel: ML Modeli Çalıştırılmaz!
        Saf Matematiksel P_10:15 Anlık Fiyat Bekçisi olarak çalışır.
        """
        last_signal = self.db_vault.get_last_signal(ticker)
        if not last_signal:
            return None # Cold start'ta dünkü stop loss yoksa sentinel pas geçer
        ...
        if p_1015 <= stop_loss_prev or pct_change <= dynamic_stop_threshold:
            logger.warning(f"[10:15 GAP SENTINEL OVERRIDE] {ticker} - P_1015: {p_1015:.2f} TL (Stop: {stop_loss_prev:.2f} TL, Değişim: %{pct_change*100:.2f})")
            return {
                "ticker": ticker,
                ...
                "reason": f"10:15 Anlık Fiyat ({p_1015:.2f} TL) Stop Loss ({stop_loss_prev:.2f} TL) veya %{dynamic_stop_threshold*100:.1f} Dinamik Bariyeri Kırdı!"
            }
        return None

    def evaluate_eod_signal(self, df_live: pd.DataFrame, ticker: str) -> Dict[str, Any]:
        """
        18:15 EOD Gün Sonu Analizi.
        Nihai Doğrulanmış Sinyal Denklemi ve 9-Durumlu Matris İşlenir.
        """
        if not any(c.endswith("_ranked") for c in df_live.columns):
        ...
        # Cold-Start Fallback Handling: If P_{t-1} is None, default P_{t-1} = P_t
        ...
        if pct_change <= dynamic_stop_threshold or p_success < 0.35 or (p_success < 0.45 and (last_signal is None or last_signal.get("days_held", 5) >= 5)):
            ...
            current_signal_code = last_state if last_state != 0 or last_signal is not None else (1 if p_success >= 0.54 else 0) # Hysteresis KORU
            revision_reason = "Hysteresis Bolgesi - Pozisyon Korundu"

        # SQLite Async Write-Queue Kaydı
...
if __name__ == "__main__":
    ...
    scheduler.initialize_models()
    
    df_live = pd.read_parquet("data/bist_2016_2026_adjusted.parquet")
    thyao_eod = scheduler.evaluate_eod_signal(df_live, "THYAO")
    print(f"\n[SCHEDULER EOD TEST SUCCESS] THYAO Net Karar: {thyao_eod['consolidated_name']} ({thyao_eod['advisory']}) | P: %{thyao_eod['p_success']*100:.1f}")
