...
def run_asels_2025_simulation():
    print("===================================================================================")
    print(" ASELS (ASELSAN) 2025 YILI 12 AYLIK TAM KÖR TEST VE GERÇEKLEŞME SİMÜLASYONU")
    print("===================================================================================")
    ...
    # 2. Filter ASELS data for 2025 (Strictly Unseen 2025 Period)
    asels_2025 = df_labeled[(df_labeled["ticker"] == "ASELS") & (df_labeled["timestamp"] >= "2025-01-01T00:00:00Z") & (df_labeled["timestamp"] <= "2025-12-31T23:59:59Z")].sort_values("timestamp").reset_index(drop=True)

    feature_cols = [c for c in asels_2025.columns if c.endswith("_ranked")]

    # Sample monthly points (first trading day of each month in 2025)
    asels_2025["dt"] = pd.to_datetime(asels_2025["timestamp"])
    monthly_points = asels_2025.groupby(asels_2025["dt"].dt.to_period("M")).first().reset_index(drop=True)
    ...
    print(f"\n===================================================================================")
    print(f" ASELS (ASELSAN) 2025 YILI SİMÜLASYON İSTATİSTİKLERİ:")
