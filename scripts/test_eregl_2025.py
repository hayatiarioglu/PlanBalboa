...
def run_eregl_2025_report():
    ...
    eregl_df = df[(df["ticker"] == "EREGL") & (df["timestamp"] >= "2025-01-01") & (df["timestamp"] <= "2025-12-31")].copy()
    eregl_df["month"] = pd.to_datetime(eregl_df["timestamp"]).dt.to_period("M")

    months = sorted(eregl_df["month"].unique())
    
    print("\n" + "="*75)
    print("🏆 EREGL (EREĞLİ DEMİR ÇELİK) 2025 YILI AY AY MODEL KANIT RAPORU")
    print("="*75 + "\n")

    correct_predictions = 0

    for m in months:
        m_df = eregl_df[eregl_df["month"] == m].sort_values("timestamp")
        ...
        curr_idx = df[(df["ticker"] == "EREGL") & (df["timestamp"] == start_row["timestamp"])].index[0]
    ...
    print("="*75)
    print(f"📊 2025 YILI EREGL TOPLAM DOGRULUK ORANI: %{acc_pct:.1f} ({len(months)} Ayin {correct_predictions} Ayinda Tam İsabet)")
    print("="*75)

if __name__ == "__main__":
    run_eregl_2025_report()
