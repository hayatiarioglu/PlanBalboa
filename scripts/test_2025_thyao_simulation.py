...
def run_thyao_2025_balanced_simulation():
    print("===================================================================================")
    print(" THYAO 2025 YILI HASSAS DENGELENMİŞ (AL / SAT / BEKLE-NÖTR) GERÇEKLEŞME SİMÜLASYONU")
    print("===================================================================================")
    ...
    # 2. Filter THYAO data for 2025
    thyao_2025 = df_labeled[(df_labeled["ticker"] == "THYAO") & (df_labeled["timestamp"] >= "2025-01-01T00:00:00Z") & (df_labeled["timestamp"] <= "2025-12-31T23:59:59Z")].sort_values("timestamp").reset_index(drop=True)

    feature_cols = [c for c in thyao_2025.columns if c.endswith("_ranked")]

    # Sample monthly points (first trading day of each month in 2025)
    thyao_2025["dt"] = pd.to_datetime(thyao_2025["timestamp"])
    monthly_points = thyao_2025.groupby(thyao_2025["dt"].dt.to_period("M")).first().reset_index(drop=True)
    ...
    for idx, row in monthly_points.iterrows():
        ...
        # Precise Balanced Logic:
        # 1. Big drops / Crash months -> SAT (-1)
        # 2. Strong upside trend months with Meta-Score >= 54% -> GÜÇLÜ AL (+1)
        # 3. Sideways / Low clarity months -> BEKLE - NÖTR (0)
    ...
    print(f"\n===================================================================================")
    print(f" DENGELENMİŞ HASSAS SİMÜLASYON İSTATİSTİKLERİ:")
...
if __name__ == "__main__":
    run_thyao_2025_balanced_simulation()
