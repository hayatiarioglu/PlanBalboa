...
def run_eregl_2025_simulation():
    print("===================================================================================")
    print(" EREGL (EREĞLİ DEMİR ÇELİK) 2025 YILI 12 AYLIK TAM KÖR TEST VE GERÇEKLEŞME SİMÜLASYONU")
    print("===================================================================================")
    ...
    # 2. Filter EREGL data for 2025 (Strictly Unseen 2025 Period)
    eregl_2025 = df_labeled[(df_labeled["ticker"] == "EREGL") & (df_labeled["timestamp"] >= "2025-01-01T00:00:00Z") & (df_labeled["timestamp"] <= "2025-12-31T23:59:59Z")].sort_values("timestamp").reset_index(drop=True)

    feature_cols = [c for c in eregl_2025.columns if c.endswith("_ranked")]

    # Sample monthly points (first trading day of each month in 2025)
    eregl_2025["dt"] = pd.to_datetime(eregl_2025["timestamp"])
    monthly_points = eregl_2025.groupby(eregl_2025["dt"].dt.to_period("M")).first().reset_index(drop=True)
    ...
    print(f"\n===================================================================================")
    print(f" EREGL (EREĞLİ DEMİR ÇELİK) 2025 YILI SİMÜLASYON İSTATİSTİKLERİ:")
