...
def predict_garan_jan_2026():
    print("===================================================================================")
    print(" GARAN (GARANTİ BANKASI) 2026 OCAK OTONOM GELECEK TAHMİNİ (PURE FORWARD PREDICTION)")
    print("===================================================================================")
    ...
    # 2. Filter GARAN data for 2026-01-01
    garan_jan_2026 = df_clean[(df_clean["ticker"] == "GARAN") & (df_clean["timestamp"] >= "2026-01-01T00:00:00Z") & (df_clean["timestamp"] <= "2026-01-31T23:59:59Z")].sort_values("timestamp").reset_index(drop=True)

    if garan_jan_2026.empty:
        print("HATA: 2026 Ocak verisi bulunamadı.")
        return

    row = garan_jan_2026.iloc[0]
    t_str = row["timestamp"][:10]
    cur_price = row["close"]

    feature_cols = [c for c in garan_jan_2026.columns if c.endswith("_ranked")]
    ...
    print(f"\n===================================================================================")
    print(f" GARAN 2026 OCAK FIRSAT KARTI (OPPORTUNITY CARD)")
    print(f" -> Tahmin Tarihi: {t_str}")
    print(f" -> Mevcut Fiyat: {round(cur_price, 2)} TL")
    print(f" -> Model Tahmin Sinyali: {decision_signal}")
    print(f" -> Guven Skoru P(Success): %{p_success * 100:.1f}")
    print(f" -> Boga P(+1) Olasiligi: %{p_bullish * 100:.1f}")
    print(f" -> Ayi P(-1) Olasiligi: %{p_bearish * 100:.1f}")
    print(f" -> Hedef Fiyat Araligi: {round(target_low, 2)} TL - {round(target_high, 2)} TL (+%7.5 ... +%9.5)")
    print(f" -> Stop Loss Seviyesi: {round(stop_loss, 2)} TL (-%4.0)")
    print(f"===================================================================================")

if __name__ == "__main__":
    predict_garan_jan_2026()
